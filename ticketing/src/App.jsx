  ...
  const tickets = useMemo(() => {
    ...
    for (const leg of legs) {
      if (leg.system === 'Local Train') {
        if (currentLocal) {
          // If we merge, the node we are currently at is a 'via' station
          const via = currentLocal.toNode.name;
          if (!currentLocal.viaNodes.includes(via)) {
            currentLocal.viaNodes.push(via);
          }
            ...
            <>
              ...
              <div className="stats-row">
                <div className="stat-chip">
                  <span className="stat-val">{tickets.length + 1}</span>
                  <span className="stat-label">Ticket{(tickets.length + 1) > 1 ? 's' : ''}</span>
                </div>
                <div className="stat-chip">
            ...
            {selectedTicket.system === 'Local Train' ? (
              <div className="uts-ticket">
                <div className="uts-header">
                  <span>{selectedTicket.system.toUpperCase()} PASS</span>
                ...
                <div className="uts-body">
                  ...
                  <div className="uts-via">
                    VIA: {selectedTicket.viaNodes.length > 0 ? `${selectedTicket.viaNodes.join(' · ').toUpperCase()} · ` : ''}{selectedTicket.lines.join(' · ').toUpperCase()}
                  </div>
              ...
              <div className="fs-ticket" style={{ borderTop: `6px solid ${SYSTEM_META[selectedTicket.system].color}` }}>
                ...
                <div className="fs-details-grid">
                  ...
                  <div className="fs-detail" style={{ gridColumn: '1 / -1' }}>
                    <span className="fs-label">VALID PATH</span>
                    <span className="fs-val">
                      {selectedTicket.fromNode.name}
                      {selectedTicket.viaNodes.map(v => ` → ${v}`)}
                      {` → ${selectedTicket.toNode.name}`}
                    </span>
                  </div>
                  <div className="fs-detail" style={{ gridColumn: '1 / -1' }}>
